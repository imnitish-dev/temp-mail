...
const InboxView = ({ username }: { username: String }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`https://mailapi.imnitish.dev/v1/mail/${username}`);
      const data: ApiResponse = await response.json();
      setData(data);
      // Reset the countdown whenever data is fetched
      setCountdown(5);
      ...
      <section className="w-full bg-gray-900">
        <div className='flex  justify-between items-center p-3'>
          ...
          <div className='flex'>
            ...
            <p className='mb-0 ms-3'>Next refresh in: {countdown} seconds</p>
          </div>
        </div>
        {data && Array.isArray(data) ? (
          <div className='h-full'>
            ...
            {view === 'list' ? (
              <>
                ...
                <table className="table-auto p-2  w-full text-left" >
                  <thead>
                    <tr className="text-white bg-[#343445]">
                      <th className="p-2 py-4">Sender</th>
                      <th className="p-2 py-4">Subject</th>
                      <th className="p-2 py-4">View</th>
                    </tr>
                  </thead>
                  <tbody className='overflow-scroll	'>
                    {data.map((row, i) => (
                      <tr key={i} className="w-min border-collapse hover:bg-[#343460] cursor-pointer" onClick={() => {
                        setEmailData(row);
                        setView('email');
                      }}>
                        <td className="p-2">
          ...
          : (
            <div className='flex justify-center items-center py-4 md:py-8 lg:py-16 lg:px-8 md:px-4 h-full'>
              <div className="text-center">
                <Image src="/empty-inbox.webp" alt="Empty Inbox" width={300} height={300} />
                <h2 className="text-xl md:text-2xl mt-4">Your inbox is empty</h2>
